...
struct Model:
    ...
    fn __init__(out self, data: Data, proj_len: Int):
        self.data = data
        self.cache = Cache()
        self.proj_len = proj_len
        self._run(proj_len)

    fn _run(mut self, proj_len:Int):
        for t in range(proj_len + 1):
            # explicitly run each function here.
            _ = self.num_pols_if(t)
            _ = self.age(t)

    fn num_pols_if(mut self, t: Int) -> Float64:
        # TODO: is raises + if t in <array> faster than try/except?
        var result: Float64
        var opt_result: Optional[Float64]
        opt_result = self.cache.num_pols_if.find(t)
        if opt_result:
    ...
    fn age(mut self, t: Int) -> Int:
        var opt_result: Optional[Int]
        var result: Int
        opt_result = self.cache.age.find(t)
        if opt_result:
            return opt_result.value()
        else:
            if t == 0:
